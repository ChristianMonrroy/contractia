...
Alineados con Multiagente_ContractIA_vs18.ipynb + salvaguardas anti-FP (v9.12).

v9.12: Restaurar salvaguardas que reducen falsos positivos
- TIPOS PERMITIDOS en cada agente (restringe tipos inventados)
- Auditor: "Si NO hay info suficiente en el grafo, NO reportar"
- Cronista: ejemplo prohibido + regla de días de subsanación
- contexto_rag restaurado como fallback cuando GraphRAG no tiene tripletas
"""
...
PROMPT_CRONISTA = PromptTemplate(
    template=(
        ...
        "- **TIPOS PERMITIDOS:** El campo `tipo` SOLO puede ser: `ERROR_PLAZOS`, `ERROR_LOGICO` "
        "o `INCONSISTENCIA_PLAZOS`. No uses ningún otro valor.\n\n"
        "# FORMATO DE SALIDA\n"
        "Generar ÚNICAMENTE el siguiente bloque de código JSON:\n\n"
        "```json\n"
        "{{\n"
